...
def move_robots(pos, vel, steps, room_size):
    ...
    for i in range(len(positions)):
        p = pos[i]
        v = vel[i]
        x = (p[0] + v[0] * steps) % width
        y = (p[1] + v[1] * steps) % height
        new_pos.append((x, y))
    
    return new_pos


def sort_into_quadrants(positions, room_size):
    width, height = room_size

    left_max = width // 2 - 1
    right_min = (width + 1) // 2
    top_max = height // 2 - 1
    bottom_min = (height + 1) // 2

    quadrant_counts = [0,0,0,0]
    for position in positions:
        if position[0] <= left_max:
            if position[1] <= top_max:
                q = 0
                quadrant_counts[q] += 1
            elif position[1] >= bottom_min:
                q = 3
                quadrant_counts[q] += 1
            else:
                pass
        elif position[0] >= right_min:
            if position[1] <= top_max:
                q = 1
                quadrant_counts[q] += 1
            elif position[1] >= bottom_min:
